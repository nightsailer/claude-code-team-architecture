...
SendMessage
    ...
    ├── 唤醒接收方进程（tmux 后端: 进程间通知; in-process 后端: 内存通知）
...
1. **消息传递机制（二次修正，通过 inbox-verify + delivery-probe 团队验证）**：
   ...
   **投递机制因后端而异（delivery-probe 团队验证）**：

   | 后端类型 | 进程模型 | 消息投递 | 唤醒通知 |
   |---------|---------|---------|---------|
   | `tmux` | 独立进程（不同 tmux pane） | 写入 inbox 文件 | Unix Domain Socket |
